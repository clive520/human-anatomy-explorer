...
## [2026-05-19] — 整合腦部與脊髓系統 (Brain & Spine System)

### 新增 (Added)
- 導入 HRA 來源的高解析度中樞神經系統模型：大腦 (`brain.glb`) 與脊髓 (`spinal_cord.glb`)。
- 在 `explorer.html` 中新增專屬的「腦部與脊髓 (Brain & Spine)」圖層開關。
- 建立 `brain_spine.js` 資料庫，支援大腦、小腦、腦幹與脊髓的雙語點擊說明。
- 擴充 `skeleton.js` 的智慧對應 (Fallback) 邏輯，自動過濾神經模型的特定前綴並自動辨識神經部位。

### 修復 (Fixed)
- **腦部與脊髓對齊校正**：
  - 套用與血管系統相同的原生縮放比例 (`1.376x`)。
  - 修正 HRA 模型與 BodyExplorer 骨架的前後朝向差異，將模型水平旋轉 180 度 (`rotation.y = Math.PI`)。
  - 精細調整高度與深度 (`Y=0.74, Z=0.0`)，使大腦完美收攏於顱骨內，且脊髓精準穿過脊椎管。
  - 加入強制快取更新機制 (`?v=2`)，解決新舊版 JavaScript 檔案的網頁快取衝突問題。
